...
class StratifiedSampler(RaySamplerBase):
    ...
    def sample_along_rays_uniform(
        ...
        # Create bins in [0, 1]
        t_bins = self.create_t_bins(num_sample + 1, device=device)  # (num_sample+1,)
        # Compute left/right edges of bins
        t_lower = t_bins[:-1]
        t_upper = t_bins[1:]

        # Randomly sample within each bin for stratification
        u = torch.rand((num_ray, num_sample), device=device)  # (num_ray, num_sample)
        t_rand = t_lower + (t_upper - t_lower) * u  # (num_sample,) broadcasted

        # Map [0, 1] t values to [near, far] for each ray
        # Expand nears/fars to (num_ray, 1)
        nears = ray_bundle.nears
        fars = ray_bundle.fars
        t_samples = self.map_t_to_euclidean(
    t_rand,
    float(ray_bundle.nears[0].item()),
    float(ray_bundle.fars[0].item())
)
        return t_samples
